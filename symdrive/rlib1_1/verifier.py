"""
Symbolic Driving Rules DSL — rlib1.1 (Four-Stage; OPERATIONS removed).
...
progressing through four stages:
  ...
  Stage 1 — PERCEPTION:  Detect and enumerate entities with typed attributes
                         (including `ego = Ego {...}` for ego state)
  Stage 2 — FACTS:       Derive high-level boolean facts directly from PERCEPTION
  Stage 3 — RULES:       Match facts to lateral + longitudinal actions
  Stage 4 — ACTION:      Selected (lateral, longitudinal) tuple

Difference vs rlib1.0:
  - The OPERATIONS stage has been removed. The parser still tolerates an
    OPERATIONS section in model output (legacy) but it is treated as a no-op.
  - Ego state is declared as a `ego = Ego {speed, acceleration, instruction}`
    entity inside PERCEPTION; the previous EgoQuery() syntax is retired.
  - SymbolicSchema reads operations.yaml only if present (rlib1.1 omits it).
...
Example output:
    ...
    PERCEPTION:
      ...
      ego = Ego {speed: Medium, acceleration: Braking, instruction: KeepForward}
...
class SymbolicSchema:
    ...
    def _init_from_rlib(self, rlib_dir: Path) -> None:
        ...
        rlib1.1: operations.yaml is optional — when absent, the schema has no
        OPERATIONS vocabulary and the parser will treat any OPERATIONS section
        in model output as a (legacy) no-op.
        """
        with open(rlib_dir / "entities.yaml") as f:
            entities = yaml.safe_load(f)
        operations_path = rlib_dir / "operations.yaml"
...
class SymbolicParser:
    ...
    def _split_sections(self, text: str) -> dict[str, str]:
        ...
        if not found:
            raise ParseError("No section headers found (expected PERCEPTION, FACTS, RULES, ACTION; OPERATIONS optional)")
