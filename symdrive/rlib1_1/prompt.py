"""
Symbolic Chain-of-Thought (CoT) prompts for driving scenario reasoning — rlib1.1.

Generates structured 4-stage symbolic prompts (PERCEPTION → FACTS → RULES → ACTION)
using the RLIB schema definitions. Used by symbolic_cot_sample_generation.py.

rlib1.1 difference vs rlib1.0:
  - Drops the OPERATIONS stage entirely (4 stages instead of 5).
  - Ego state is declared as an entity inside PERCEPTION (e.g.
    `ego = Ego {speed: Slow, acceleration: Braking, instruction: KeepForward}`)
    rather than queried via EgoQuery(...).
"""
...
from .verifier import SymbolicSchema
...
def _load_rlib_prompt_sections(rlib_dir: str) -> dict[str, str]:
    ...
    entity_types_text = (
        ...
        "2. Ego state goes in PERCEPTION as an entity, e.g.\n"
        "   `ego = Ego {speed: Slow, acceleration: Braking, instruction: KeepForward}`\n"
        "   (This replaces the EgoQuery() syntax from the previous design.)\n"
        ...
        "  Ego.speed: Stopped/Slow/Medium/Fast    Ego.acceleration: Braking/Coasting/Accelerating\n"
        "  Ego.instruction: KeepForward/TurnLeft/TurnRight\n"
        + "\n".join(entity_lines)
    )
...
PERCEPTION:
  ...
  ego = Ego {speed: Medium, acceleration: Coasting, instruction: KeepForward}  # raw: 6.5 m/s, 0.10 m/s^2
...
PERCEPTION:
  ...
  r_1 = Intersection {position: Ahead, distance: Near}
  ego = Ego {speed: Medium, acceleration: Braking, instruction: KeepForward}  # raw: 7.2 m/s, -0.80 m/s^2
...
PERCEPTION:
  ...
  ego = Ego {speed: Medium, acceleration: Coasting, instruction: KeepForward}  # raw: 5.4 m/s, -0.20 m/s^2
...
def get_symbolic_cot_prompt(
    ...
    prompt_text = (
        ...
        "r_ for RoadFeature/subtypes (Crosswalk,Intersection...). Use the identifier `ego` for "
        "the Ego entity (exactly one per scene).\n"
        f"{sections['entity_types']}\n\n"

        "=== FACT VOCABULARY ===\n"
        "Set each relevant fact to True or False based on your perception.\n"
        ...
        "=== EGO VEHICLE STATE (declare in PERCEPTION as an Ego entity) ===\n"
        f"ego = Ego {{speed: {ego_speed['label']}, "
        f"acceleration: {ego_acceleration['label']}, "
        f"instruction: {ego_instruction}}}"
        f"  # raw: {ego_speed['raw']:.1f} {ego_speed['unit']}, "
        f"{ego_acceleration['raw']:.2f} {ego_acceleration['unit']}\n\n"
        ...
        + (
            ...
            "Do NOT copy it verbatim — translate it into PERCEPTION/FACTS/RULES/ACTION.\n\n"
            f"{nl_cot_reference}\n\n"
        ...
        "  Step 1 — Write PERCEPTION (including the Ego entity above) and FACTS purely from the scene "
        "images and the given ego state. Do NOT let the hint influence what you perceive or what "
        "facts you declare.\n"
        + (
        ...
        "  PERCEPTION:   — list detected entities (incl. ego = Ego {...}) with typed attributes\n"
        "  FACTS:        — boolean facts derived directly from PERCEPTION\n"
        ...
        "6. PERCEPTION MUST include the Ego entity exactly as given in '=== EGO VEHICLE STATE ===' above.\n\n"
        ...
        "Output ONLY the four sections (PERCEPTION, FACTS, RULES, ACTION), "
        "no additional text."
    )
