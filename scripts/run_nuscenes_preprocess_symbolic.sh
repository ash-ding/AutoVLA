#!/usr/bin/env bash
# Generate symbolic CoT JSONs for a nuScenes bucket via symbolic_cot_sample_generation.py.
#
# Env overrides (all optional):
#   CONFIG_NAME         hydra-style dataset config name (without .yaml).
#                       Default: dataset/symbolic-cot-gpt4o-mini-nuscenes-trainval
#                       (legacy gpt-4o-mini; rlib1.0 ablation uses
#                       dataset/qwen2.5-vl-72B-nuscenes-symbolic-{4v,3v})
#   OUTPUT_DIR          where per-token JSONs land.
#                       Default: /data/nuscenes_symbolic_reasoning_samples_gpt_4o_mini_2895
...
#   SAMPLE_IDS_KEY      bucket name. Default: nuscenes_cot
...
#   TP_SIZE             vLLM tensor-parallel size. Default: omitted
...
CONFIG_NAME="${CONFIG_NAME:-dataset/symbolic-cot-gpt4o-mini-nuscenes-trainval}"
OUTPUT_DIR="${OUTPUT_DIR:-/data/nuscenes_symbolic_reasoning_samples_gpt_4o_mini_2895}"
SCALING_TOKEN_LIST="${SCALING_TOKEN_LIST:-/data/nuplan_nuscenes_train_mix_185k/scaling_185k_token_list.json}"
SAMPLE_IDS_KEY="${SAMPLE_IDS_KEY:-nuscenes_cot}"
...
LOG="$LOG_DIR/run_nuscenes_preprocess_symbolic_${LOG_NAME}"

echo "===== [run_nuscenes_preprocess_symbolic] config=${CONFIG_NAME} out=${OUTPUT_DIR} =====" | tee -a "$LOG"
...
echo "===== [run_nuscenes_preprocess_symbolic] complete =====" | tee -a "$LOG"
